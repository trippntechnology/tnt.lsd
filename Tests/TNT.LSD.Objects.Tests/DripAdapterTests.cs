...
public class DripAdapterTests
{
  ...
  {
    var sut = new DripAdapter();
    Assert.That(sut.Stub, Is.EqualTo(StubType.PVC));

    sut.Stub = StubType.FunnyPipe;
    Assert.That(sut.Stub, Is.EqualTo(StubType.FunnyPipe));

    var newSut = new DripAdapter(sut);
    Assert.That(newSut.Stub, Is.EqualTo(StubType.FunnyPipe));

    var clone = newSut.Clone() as DripAdapter;
    Assert.That(clone, Is.Not.Null);
    Assert.That(clone.Stub, Is.EqualTo(StubType.FunnyPipe));
  }
}
