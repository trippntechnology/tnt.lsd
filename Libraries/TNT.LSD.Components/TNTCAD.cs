...
public partial class TNTCAD : Control
{
  ...
  protected DrawingMode m_DrawingMode = new DrawingModes.NullMode();
  ...
  public LSDSettings Settings { get { return m_State.Settings; } }
  ...
  private Type[] ExpectedTypes
  {
    get
    {
      if (m_ExpectedTypes == null)
      {
        ...
        Assembly settingsAss = Assembly.LoadFile(string.Concat(Application.StartupPath, "\\", "TNT.LSD.Settings.dll"));
        List<Type> types = new List<Type>();
        types.AddRange(Utilities.Utilities.GetTypes(objectsAss, t => t.Namespace == "TNT.LSD.Objects" && !t.IsAbstract && t.InheritsFrom(typeof(TNTObject))));
        types.AddRange(Utilities.Utilities.GetTypes(objectsAss, t => t.Namespace == "TNT.LSD.Objects.ControlPoints" && !t.IsAbstract && t.InheritsFrom(typeof(TNTControlPoint))));
        types.AddRange(Utilities.Utilities.GetTypes(settingsAss, t => t.Namespace == "TNT.LSD.Settings" && !t.IsAbstract && t.InheritsFrom(typeof(LSDSettings))));
  ...
  {
    int x1 = System.Math.Min(p1.X, p2.X);
    int y1 = System.Math.Min(p1.Y, p2.Y);
    int width = System.Math.Abs(x1 - System.Math.Max(p1.X, p2.X));
    int height = System.Math.Abs(y1 - System.Math.Max(p1.Y, p2.Y));
  ...
  public List<Part> GetPartsList()
  {
    ...
    if (gravelPart != null)
    {
      gravelPart.Quantity = System.Math.Ceiling(gravelPart.Quantity);
    }
