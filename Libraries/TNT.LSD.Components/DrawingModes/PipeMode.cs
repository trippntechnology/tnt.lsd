...
public abstract class PipeMode<T> : DrawingMode where T : Pipe, new()
{
  ...
  public override void OnMouseClick(TNTCAD cad, MouseEventArgs e, Keys modifierKeys)
  {
    ...
    else if (e.Button == MouseButtons.Right && m_PipeSegment != null)
    {
      m_PipeSegment.RemovePartsPipeRelationship();
  ...
  private List<PalettePart> GetPartsUnderPipeSegment(TNTCAD cad, Pipe pipeSegment, BasePart endOfSegment)
  {
    ...
    }).Cast<PalettePart>().ToList();

    // Order closest to farthest from startOfSegment
    partsUnderSegment = (from p in partsUnderSegment orderby p.Position.Distance(startOfSegment.Position) select p).ToList();
  ...
  protected virtual TNTObject GetObjectUnderMouse(TNTCAD cad, MouseEventArgs e, Keys modifierKeys)
  {
    Point currentPos = new Point(e.X, e.Y).ToWorldCoordinateSpace(cad.CreateGraphics(), false);
